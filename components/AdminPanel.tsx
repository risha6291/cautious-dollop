...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const handleUpdateBannerImage = async (bannerId: string, newImageUrl: string) => {
    ...
    try {
      ...
      // যদি URL খালি থাকে, তাহলে movie এর main thumbnail use করবে
      let finalImage = newImageUrl.trim();
      if (!finalImage && banner.movieId) {
        const movie = movieList.find(m => m.id === banner.movieId);
        if (movie) {
          // Priority: detailBanner > thumbnail
          finalImage = movie.detailBanner || movie.thumbnail;
        }
      }
          ...
          {activeTab === 'banners' && (
            <div className="space-y-6">
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          {isEditingImg && (
                            <div className="bg-gray-900/50 border border-blue-500/30 rounded-lg p-3 space-y-3">
                              ...
                              />
                              {newImageUrl && newImageUrl !== banner.image && (
                                <div className="relative">
                                  <img 
                                    src={newImageUrl} 
                                    alt="Preview" 
                              ...
                              <div className="flex gap-2">
                                ...
                                >
                                  {newImageUrl ? 'Save Custom URL' : 'Remove Custom (Use Main Thumbnail)'}
                                </button>
                                <button
                                  onClick={() => {
              ...
              <div>
                ...
                <div className="bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-500/30 rounded-xl p-4 mb-4 space-y-3">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-blue-300 text-sm font-bold">🖼️ Custom Banner Image (Optional)</p>
                    <span className="text-[10px] bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full">খালি রাখতে পারেন</span>
                  </div>
                  
                  <div className="bg-gray-900/50 rounded-lg p-3 space-y-2">
                    <div className="flex items-start gap-2 text-xs text-gray-400">
                      <span className="text-green-400 shrink-0">✓</span>
                      <span><strong className="text-white">URL দিলে:</strong> এই custom image home এর বড় banner এ দেখাবে</span>
                    </div>
                    <div className="flex items-start gap-2 text-xs text-gray-400">
                      <span className="text-green-400 shrink-0">✓</span>
                      <span><strong className="text-white">URL না দিলে:</strong> Movie এর main thumbnail বড় banner এ দেখাবে</span>
                    </div>
                    <div className="flex items-start gap-2 text-xs text-gray-400">
                      <span className="text-yellow-400 shrink-0">→</span>
                      <span><strong className="text-white">Click করলে:</strong> সব সময় movie এর details/download page এ যাবে</span>
                    </div>
                    <div className="flex items-start gap-2 text-xs text-gray-400">
                      <span className="text-purple-400 shrink-0">📝</span>
                      <span><strong className="text-white">Collection cards:</strong> আগের main thumbnail-ই থাকবে (unchanged)</span>
                    </div>
                  </div>
                  ...
                  <input
                    ...
                    placeholder="https://i.ibb.co/custom-banner.jpg (16:9 landscape) — Optional"
